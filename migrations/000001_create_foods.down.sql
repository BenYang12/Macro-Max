...
CREATE TABLE foods(
    ...
    -- TEXT[] is a Postgres ARRAY column: dietary flags like
    -- {'vegetarian','gluten_free'}. DEFAULT '{}' = empty array, never NULL.
...
-- A GIN (Generalized Inverted) index makes array-containment queries fast:
-- WHERE tags @> '{vegan}' ("tags contains vegan"). A normal btree index
-- can't look INSIDE an array; GIN indexes each element.